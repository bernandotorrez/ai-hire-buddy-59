...
export const Route = createFileRoute("/jobs")({
  head: () => ({ meta: [{ title: "Browse Jobs — WarmHire" }, { name: "description", content: "Discover open roles and apply with your AI-built profile." }] }),
  component: JobsList,
});
...
function JobsList() {
  ...
  useEffect(() => {
    supabase
      .from("jobs")
      .select("id, title, company, location, employment_type, description, salary_min, salary_max, created_at")
      .eq("status", "open")
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-muted-foreground">{filtered.length} {filtered.length === 1 ? "role" : "roles"} waiting for you.</p>
          </div>
        ...
        {loading ? (
          ...
          <Card className="p-10 text-center text-muted-foreground">No jobs found. Check back soon!</Card>
        ) : (
                ...
                <Card className="group p-6 transition-all hover:-translate-y-0.5 hover:shadow-warm">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="flex-1">
                      <h2 className="font-display text-xl font-bold transition-colors group-hover:text-primary">{j.title}</h2>
                      <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                        <span className="flex items-center gap-1"><Building2 className="h-4 w-4" /> {j.company}</span>
                        {j.location && <span className="flex items-center gap-1"><MapPin className="h-4 w-4" /> {j.location}</span>}
                        {j.employment_type && <span className="flex items-center gap-1"><Briefcase className="h-4 w-4" /> {j.employment_type}</span>}
                      </div>
